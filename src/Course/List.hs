...
flatten ::
  ...
  -> List a
flatten Nil = Nil
flatten (xs :. xss) = foldLeft (++) xs xss
...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional xs =
   if (containsEmpty xs) then Empty
                         else Full (map unpack xs)
      where unpack (Full x) = x
            containsEmpty xs = not $ isOptEmpty $ find isOptEmpty xs 
            p acc x = acc || isOptEmpty x

isOptEmpty Empty = True
isOptEmpty _ = False
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = not $ isEmpty $ snd $ splitAt (4 - 1) xs

splitAt 0 xs = (Nil, xs)
splitAt _ Nil = (Nil, Nil)
splitAt n (x :. xs) = let (y,z) =  splitAt (n-1) xs 
                      in (x :. y , z)
...
notReverse ::
  List a
  -> List a
notReverse (x :. y :. ss) = y :. x :. ss
notReverse xs = xs -- length 0 and 1
